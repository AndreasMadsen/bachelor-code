...
def count_decoder_sequence(items, T=8, classes=5, Tmax=15):
    ...
    # add <EOS>
    t = t + 1
    t_pad = np.zeros((items, Tmax), dtype='int32')
    t_pad[:, 0:t.shape[1]] = t

    return ((X / classes).astype('float32'), t_pad.astype('int32'))

def count_network_sequence(items, T=8, classes=5, Tmax=15):
    # Create initial value
    ...
    return (X.astype('float32'), t_pad.astype('int32'))
